...
def lti_launch():
    ...
    for param in required_params:
        if param not in request.form:
            return f"LTI Launch Failed: Missing required parameter: {param}", 403
    
    # Validate consumer key
    if request.form.get('oauth_consumer_key') != LTI_CONSUMER_KEY:
        return "LTI Launch Failed: Invalid consumer key", 403
